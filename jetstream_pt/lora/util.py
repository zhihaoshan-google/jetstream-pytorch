...
def apply_lora(
  ...
  selected_batched_lora_a = batched_lora_a.index_select(indices)
  selected_batched_lora_b = batched_lora_b.index_select(indices)
  selected_scaling = batched_scaling.index_select(indices)[:, None, None]
  output += (
      torch.bmm(
